...
A saved brand is a JSON document: the ramps the eye drew, which ramp plays
which shadcn role, and the steps moved off the preset. Everything the studio
shows or ships is a pure function of that document, in `apps/studio/src/lib`.
A role is the only way a ramp reaches a token: `neutral`, `primary`,
`destructive`, `secondary`, `accent`, `sidebar`, `ring`, and the five chart
series each name a ramp, and a token's ramp is its role's. The eye moves
steps, not ramps. Nothing ships unless every token clears. There is no
fallback color.
...
- `apps/studio` — the brand palette studio: React, Vite, Tailwind v4, shadcn/ui.
  Its `src/lib` holds the brand document and what ships from it.
